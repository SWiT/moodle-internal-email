<?php // $Id: tablelib.php,v 1.6.2.4 2005/11/13 04:45:23 defacer Exp $
...
class email_flexible_table {
    ...
    function setup() {
        ...
        $this->currpage = optional_param($this->request[TABLE_VAR_PAGE], 0, PARAM_ALPHANUM);
        $this->setup = true;
    }
    ...
    function print_html() {
        ...
        foreach($this->columns as $column => $index) {
            ...
            if($this->is_collapsible) {
                if(!empty($this->sess->collapse[$column])) {
                    $icon_hide = ' <a href="'.$this->baseurl.$this->request[TABLE_VAR_SHOW].'='.$column.'"><img src="'.$OUTPUT->pix_url('t/switch_plus').'" title="'.get_string('show').' '.$this->headers[$index].'" alt="'.get_string('show').' '.$this->headers[$index].'" /></a>';
                }
                else if($this->headers[$index] !== NULL) {
                    $icon_hide = ' <a href="'.$this->baseurl.$this->request[TABLE_VAR_HIDE].'='.$column.'"><img src="'.$OUTPUT->pix_url('t/switch_minus').'" title="'.get_string('hide').' '.$this->headers[$index].'" alt="'.get_string('show').' '.$this->headers[$index].'" /></a>';
                }
            }
            ...
            switch($column) {
                ...
                if($this->is_sortable) {
                    ...
                    if($primary_sort_column == 'firstname') {
                        if($primary_sort_order == SORT_ASC) {
                            $icon_sort_first = ' <img src="'.$OUTPUT->pix_url('t/down').'" alt="down" />';
                        }
                        else {
                            $icon_sort_first = ' <img src="'.$OUTPUT->pix_url('t/up').'" alt="up" />';
                        }
                    }
                    else if($primary_sort_column == 'lastname') {
                        if($primary_sort_order == SORT_ASC) {
                            $icon_sort_last = ' <img src="'.$OUTPUT->pix_url('t/down').'" alt="down" />';
                        }
                        else {
                            $icon_sort_last = ' <img src="'.$OUTPUT->pix_url('t/up').'" alt="up" />';
                        }
                    }
                ...
                if($this->is_sortable) {
                    if($primary_sort_column == $column) {
                        if($primary_sort_order == SORT_ASC) {
                            $icon_sort = ' <img src="'.$OUTPUT->pix_url('t/down').'" alt="down" />';
                        }
                        else {
                            $icon_sort = ' <img src="'.$OUTPUT->pix_url('t/up').'" alt="up" />';
                        }
                    }
        ...
        if(!empty($this->data)) {
            ...
            foreach($this->data as $row) {
                $oddeven = $oddeven ? 0 : 1;
                echo '<tr '.$this->make_attributes_string($row->attributes).' class="r'.$oddeven.'">';
